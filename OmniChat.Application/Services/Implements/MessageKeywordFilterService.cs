...
namespace OmniChat.Application.Services.Implements
{
    public class MessageKeywordFilterService : BaseService<MessageKeywordFilterService>, IMessageKeywordFilterService
    {
        ...
        //        result.Recommends.Add(new IsRecommentOnMesssageResponse
        //        {
        //            RecommendType = RecommendType.CreateOrder,
        //            Data = null 
        //        });
        //    }
        ...
        public async Task<ExtractKeywordResponse> ExtractKeywords(string content, List<string>? productNames = null)
        {
            ...
            var phoneNumbers = PhoneRegex.Matches(content).Select(m => m.Value).Distinct().ToList();
            var emails = EmailRegex.Matches(content).Select(m => m.Value).Distinct().ToList();

            if (phoneNumbers.Any() || emails.Any())
            {
        
                var customers = await customerProfileRepo.GetQueryable()
                    .Where(cp => phoneNumbers.Contains(cp.PhoneNumber) || emails.Contains(cp.Email))
                    .ToListAsync();

                foreach (var phone in phoneNumbers)
                {
                    result.Highlights.Add(phone);
                    var customer = customers.FirstOrDefault(c => c.PhoneNumber == phone);
                    result.Recommends.Add(new IsRecommentOnMesssageResponse
                    {
                        RecommendType = RecommendType.SearchCustomerInfo,
                        Data = customer == null ? null : new SearchCustomerInfoRecommendData
                        {
                ...
                {
                    result.Highlights.Add(email);
                    if (!result.Recommends.Any(r => r.RecommendType == RecommendType.SearchCustomerInfo &&
                        ((SearchCustomerInfoRecommendData)r.Data)?.CustomerEmail == email))
                    {
            ...
            var productCodes = ProductCodeRegex.Matches(content).Select(m => m.Value.ToUpper()).Distinct().ToList();
            if (productCodes.Any())
            {
                var validProducts = await productRepo.GetQueryable()
          .Where(p => productCodes.Contains(p.Code) && p.IsActive == true)
          .ToListAsync();
            ...
            var orderCodes = OrderCodeRegex.Matches(content).Select(m => m.Value.ToUpper()).Distinct().ToList();
            if (orderCodes.Any())
            {
                var validOrders = await orderRepo.GetQueryable()
                .Include(o => o.CustomerProfile)
                .Where(o => orderCodes.Contains(o.Code) && o.IsDeleted == false)
                .ToListAsync();
